...
import {
  ...
  toBytes,
  toHex
} from "./chunk-L6ATHVBP.js";
import "./chunk-C4TDMKNG.js";
...
var deriveExternalMessageCallKey = (selectorOrCalldata = "0x") => {
  const bytes = typeof selectorOrCalldata === "string" ? hexToBytes(selectorOrCalldata) : selectorOrCalldata;
  if (bytes.length < 4) {
...
var normalizeTransactionFees = (fees) => {
  const distribution = createFeesDistribution(fees == null ? void 0 : fees.distribution);
  const messageAllocations = normalizeMessageFeeAllocations(fees == null ? void 0 : fees.messageAllocations);
  const feeValue = (fees == null ? void 0 : fees.feeValue) === void 0 ? void 0 : toUInt(fees.feeValue, "fees.feeValue");
  return {
    distribution,
    messageAllocations,
    feeValue,
...
function extractGenCallFeeReport(feeAccounting) {
  const report = feeAccounting == null ? void 0 : feeAccounting.execution_fee_report;
...
var contractActions = (client, publicClient) => {
  const estimateFeeValue = async (distribution, policy) => {
    var _a;
    if ((_a = client.chain.feeManagerContract) == null ? void 0 : _a.address) {
  ...
  return {
    ...
    estimateTransactionFees: async (args) => {
      ...
      return {
        distribution,
        messageAllocations: args == null ? void 0 : args.messageAllocations,
...
var _resolveAddressManagerAddress = async ({
  ...
  if (!(consensusMainContract == null ? void 0 : consensusMainContract.address)) {
...
var extractStudioFeePolicy = (config) => {
  ...
  const policy = configRecord == null ? void 0 : configRecord.policy;
  const enabled = configRecord == null ? void 0 : configRecord.enabled;
...
var buildEstimatedFeesDistribution = (options, policy, defaultConsensusMaxRotations) => {
  const headroomBps = toUInt2(
    options == null ? void 0 : options.priceCapHeadroomBps,
    "priceCapHeadroomBps",
    DEFAULT_PRICE_CAP_HEADROOM_BPS
  );
  const baseExecutionBudgetDefault = defaultExecutionBudgetPerRound(policy);
  const messageAllocations = (options == null ? void 0 : options.messageAllocations) ? normalizeMessageFeeAllocations(options.messageAllocations) : void 0;
  const totalMessageFees = (options == null ? void 0 : options.totalMessageFees) ?? (messageAllocations ? messageAllocations.reduce(
    (sum, allocation) => {
  ...
  const emitsMessages = ((messageAllocations == null ? void 0 : messageAllocations.length) ?? 0) > 0 || totalMessageFees !== void 0 && toUInt2(totalMessageFees, "totalMessageFees", 0n) > 0n;
  ...
  const appealRounds = toUInt2(options == null ? void 0 : options.appealRounds, "appealRounds", 0n);
  ...
  let rotations = options == null ? void 0 : options.rotations;
  if (rotations === void 0) {
  ...
  return createFeesDistribution({
    leaderTimeunitsAllocation: (options == null ? void 0 : options.leaderTimeunitsAllocation) ?? (policy.enabled ? DEFAULT_LEADER_TIMEUNITS_ALLOCATION : 0n),
    validatorTimeunitsAllocation: (options == null ? void 0 : options.validatorTimeunitsAllocation) ?? (policy.enabled ? DEFAULT_VALIDATOR_TIMEUNITS_ALLOCATION : 0n),
    appealRounds,
    executionBudgetPerRound: (options == null ? void 0 : options.executionBudgetPerRound) ?? executionBudgetDefault,
    executionConsumed: options == null ? void 0 : options.executionConsumed,
    totalMessageFees,
    rotations,
    maxPriceGenPerTimeUnit: (options == null ? void 0 : options.maxPriceGenPerTimeUnit) ?? withCapHeadroom(policy.genPerTimeUnit, headroomBps),
    storageFeeMaxGasPrice: (options == null ? void 0 : options.storageFeeMaxGasPrice) ?? withCapHeadroom(policy.storageUnitPrice, headroomBps),
    receiptFeeMaxGasPrice: (options == null ? void 0 : options.receiptFeeMaxGasPrice) ?? withCapHeadroom(policy.receiptGasPrice, headroomBps)
  });
};
...
var feeAccountingFromSimulation = (simulation) => {
  ...
  const genvmResult = asRecord(receipt == null ? void 0 : receipt.genvm_result);
  const feeAccounting = asRecord(genvmResult == null ? void 0 : genvmResult.fee_accounting);
  return feeAccounting;
};
var messageAllocationsFromAccounting = (accounting) => {
  if (!Array.isArray(accounting == null ? void 0 : accounting.message_allocations) || accounting.message_allocations.length === 0) {
    return void 0;
  }
...
var observedSimulationFeeUsage = (args, policy) => {
  ...
  const report = args.simulation.feeReport ?? (accounting == null ? void 0 : accounting.execution_fee_report);
  ...
  const executionFeeConsumed = bigintFromUnknown(
    accounting == null ? void 0 : accounting.execution_fee_consumed,
    "simulation.feeAccounting.execution_fee_consumed"
  );
  const executionFeeReportTotal = bigintFromUnknown(
    report == null ? void 0 : report.totalEstimatedFee,
...
var transactionFeeEstimateFromStudioEstimate = (result, policy) => {
  ...
  const preset = asRecord(estimate == null ? void 0 : estimate.recommendedPreset);
  const distributionInput = asRecord(preset == null ? void 0 : preset.distribution);
  ...
  const feeAccounting = asRecord(estimate == null ? void 0 : estimate.feeAccounting);
  const feeReport = asRecord(estimate == null ? void 0 : estimate.feeReport) ?? asRecord(feeAccounting == null ? void 0 : feeAccounting.execution_fee_report);
  return {
...
var _readLifecycleIdentity = async ({
  ...
  const consensusDataAddress = (_a = client.chain.consensusDataContract) == null ? void 0 : _a.address;
...
var _waitForSentEnvelope = async ({
  ...
  if (client.chain.isStudio) {
    try {
      ...
      const reason = (rawReceipt == null ? void 0 : rawReceipt.revertReason) ?? (rawReceipt == null ? void 0 : rawReceipt.error);
...
var _sendTransaction = async ({
  ...
  const stringifyRpcError = (error) => {
    ...
    const selectorName = (_a2 = Object.entries(knownRevertSelectorNames).find(([selector]) => text.includes(selector))) == null ? void 0 : _a2[1];
...
var isSuccessful = (transaction) => {
  const statusName = transaction.statusName ?? (typeof transaction.status === "string" && transaction.status in TransactionStatus ? transaction.status : transaction.status === void 0 ? void 0 : transactionsStatusNumberToName[String(transaction.status)]);
  const executionResultName = transaction.txExecutionResultName ?? (transaction.txExecutionResult === void 0 ? void 0 : executionResultNumberToName[String(transaction.txExecutionResult)]);
  return (statusName === "ACCEPTED" || statusName === "FINALIZED") && executionResultName === "FINISHED_WITH_RETURN";
};
...
var stakingActions = (client, publicClient) => {
  ...
  const getValidatorRegistrationContext = async () => {
    ...
    if (!(consensusMain == null ? void 0 : consensusMain.address) || consensusMain.address === zeroAddress) {
...
var vestingActions = (client, publicClient) => {
  ...
  const resolveVestingFactoryAddress = async (options) => {
    if (options == null ? void 0 : options.factory) return options.factory;
    const addressManager = await getAddressManagerAddress(options == null ? void 0 : options.addressManager);
...
var createClient2 = (config = { chain: localnet }) => {
  ...
  const clientWithBasicActions = baseClient.extend(publicActions).extend(walletActions).extend((client) => accountActions(client, publicClient));
